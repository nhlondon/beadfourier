...
void getQXi(double **q, double ***a, double ***qxi, double **radxi, double *xi,
    int numPart, int kmax, int dim, int nxi)
{
  int iup;
  double pi; 
  ...
  for(int i=0; i<numPart; i++)
  {
    ...
    for(int l=0; l<nxi; l++)
    {
      ...
      for(int j=0; j<dim; j++)
      {
        qxi[l][i][j] = q[i][j] + (q[iup][j]-q[i][j])*xi[i];
        for(int k=0; k<kmax; k++)
          qxi[l][i][j] += a[i][k][j]*sin((double)(k+1)*pi*xi[k]);
        radxi[l][i] += qxi[l][i][j]*qxi[l][i][j];
      }
      radxi[l][i] = sqrt(radxi[l][i]);
    }
  }
...
void getFourierForce(double ***a, double*** aforce, double kbead,
    int numPart, int kmax, int dim)
{
  double pi;
  ...
  for(int i=0; i<numPart; i++)
  {
    ...
    {
      for(int k=0; k<dim; k++)
        aforce[i][j][k] += 0.5*kbead*((double)(k+1)*pi)*((double)(k+1)*pi)
          *a[i][j][k];
    }
  }
...
void getActionForce(double ***qxi, double** radxi, double** qforce, double*** aforce,
  double* xi, double rEq, int numPart, int kmax, int nxi, int dim, int potType)
{
  int idown;
  double pi;
  double ***xiforce, *fScalar, *tempForce;

  pi = 4.0*atan(1.0);

  fScalar = new double[numPart];
  tempForce = new double[nxi];
  xiforce = new double**[nxi];
  for(int i=0; i<nxi; i++)
  {
    xiforce[i] = new double*[numPart];
    for(int j=0; j<numPart; j++)
        xiforce[i][j] = new double[dim];
  }

  for(int i=0; i<nxi; i++)
    getForce(radxi[i],qxi[i],xiforce[i],fScalar,rEq,numPart,dim,potType);
  ...
  for(int i=0; i<numPart; i++)
  {
    ...
    {
      for(int l=0; l<nxi; l++)
        tempForce[l] = xiforce[l][i][j]*(1.0-xi[l]);
      qforce[i][j] += trapezoid(xi,tempForce,nxi)/((double) numPart);
      if(i == 0)
      {
        idown = numPart-1;
      }
      ...
      for(int l=0; l<nxi; l++)
        tempForce[l] = xiforce[l][idown][j]*xi[l];
      qforce[i][j] += trapezoid(xi,tempForce,nxi)/((double) numPart);

      for(int k=0; k<kmax; k++)
      {
        for(int l=0; l<nxi; l++)
          tempForce[l] = xiforce[l][i][j]*sin((double)(k+1)*pi*xi[k]);
        aforce[i][j][k] += trapezoid(xi,tempForce,nxi)/((double) numPart);
      }
    }
  }
